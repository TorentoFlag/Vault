...
- Current backend coverage: Steam session foundation, backend-fed catalog/pricing, authenticated server cart, initial Coins wallet ledger/holds, checkout order creation from wallet balance, current-user order history, durable Arc Pay top-up intents with immutable Coins/RUB terms, real Arc Pay sandbox Hosted Checkout creation with SBP-only request payload, and deterministic fake Arc Pay checkout/webhook processing for local inbox/posting tests.
- Current frontend limitation: no production Arc Pay webhook crediting, no real Steam OpenID browser acceptance, no real SIH purchase/refill, and inventory/trade/payment history still use local concept state. Steam login now starts the backend Steam OpenID flow; Email remains a local fallback until a production email-auth decision exists. For backend-cookie sessions, saved Steam Trade URL is kept backend-side and the frontend only shows configured/not configured status; purchase history is loaded from `/orders/me`; top-up form creates `/payments/top-up/sessions` records and redirects only when the backend returns a checkout URL. Coins are credited only after verified webhook/reconciliation processing; the current fake provider proves local idempotency and wallet posting, not real Arc Pay acceptance.
