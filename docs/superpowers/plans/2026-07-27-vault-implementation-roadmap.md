...
- [x] Implement inventory projection and only enable actions backed by real transitions. Current status: `/inventory/me` returns only the current user's fulfilled skin order lines that do not already have a withdrawal request; withdraw-to-Steam is enabled only when backend Steam Trade URL is configured; sell-to-site remains disabled until a provider-backed valuation/settlement path is approved.
- [x] Implement backend-owned inventory withdrawal request creation. Current status: `/inventory/me/items/{itemId}/withdrawals` creates an idempotent durable `steam_inventory_withdrawal` command, removes the item from the owned inventory projection, and does not claim a provider trade was sent.
- [x] Implement customer-visible Steam trade/fulfillment history projection. Current status: `/fulfillment/me/trades` returns only the current user's skin fulfillment events and withdrawal requests, excludes provider snapshots/secrets, maps monotonic fulfillment states to customer statuses, and frontend backend-cookie sessions load it into the account Steam Trade log.
...
- [ ] Implement minimal admin read models and reasoned recovery commands. Current status: `GET /admin/operations/overview` exposes token-gated, read-only, redacted queues for manual-review payments, problem orders, fulfillment commands, and unprocessed/rejected webhook events; `POST /admin/operations/payments/reconcile` runs Arc Pay pending top-up reconciliation as a reasoned, idempotent, audited command without direct balance/order overwrites. Other recovery commands remain unimplemented until their command/audit policies are written.
