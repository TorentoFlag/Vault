...
- Implemented Vault Steam refill fulfillment scope: checkout creates durable `sih_steam_refill` commands, the fulfillment worker creates a `steam_check` attempt before `POST /steam/check`, stores the returned SIH `transactionId`, creates a `steam_pay` attempt before `POST /steam/pay`, and completes the order line plus wallet hold after successful SIH pay. If pay fails after a successful check, retry reuses the saved `transactionId` instead of issuing a fresh check. This is deterministic fake-HTTP coverage, not real SIH refill acceptance.
