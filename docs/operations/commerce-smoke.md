...
- A Steam-linked customer session can create an payment provider top-up session.
- payment provider top-up reconciliation credits Coins only after a captured provider payment is observed.
...
- Captured request evidence does not include payment provider secret keys. Persisted fulfillment/provider snapshots do not expose Steam Trade URL secrets.
...
- It does not replace the SIH sandbox catalog acceptance test or the payment provider trusted-HTTPS browser/webhook acceptance path.
